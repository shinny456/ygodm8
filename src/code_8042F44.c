...
unsigned char NumLowerAtkMonInRow (unsigned char row, unsigned short atk) {
  ...
  for (i = 0; i < 5; i++) {
    if (gTurnZonePtrs[row][i]->id == CARD_NONE || !gTurnZonePtrs[row][i]->isFaceUp)
      continue;
    gStatMod.card = gTurnZonePtrs[row][i]->id;
    gStatMod.field = gDuel.field;
    gStatMod.stage = GetFinalStage(gTurnZonePtrs[row][i]);
    SetFinalStat(&gStatMod);
...
unsigned char GetNumFaceUpLockedCardsInRow (unsigned char row) {
  ...
  for (i = 0; i < 5; i++)
    if (gTurnZonePtrs[row][i]->id != CARD_NONE && gTurnZonePtrs[row][i]->isFaceUp)
      if (gTurnZonePtrs[row][i]->isLocked)
        count++;
  return count;
}
...
unsigned GetTotalAtkAndDefInRow (unsigned char row) {
  ...
  unsigned char i;
  for (i = 0; i < 5; i++) {
    if (GetTypeGroup(gTurnZonePtrs[row][i]->id) != 1)
      continue;
    gStatMod.card = gTurnZonePtrs[row][i]->id;
    gStatMod.field = gDuel.field;
    gStatMod.stage = GetFinalStage(gTurnZonePtrs[row][i]);
...
unsigned GetTotalFaceUpAtkAndDefInRow (unsigned char row) {
  ...
  unsigned char i;
  for (i = 0; i < 5; i++) {
    if (GetTypeGroup(gTurnZonePtrs[row][i]->id) != 1 || !gTurnZonePtrs[row][i]->isFaceUp)
      continue;
    gStatMod.card = gTurnZonePtrs[row][i]->id;
    gStatMod.field = gDuel.field;
    gStatMod.stage = GetFinalStage(gTurnZonePtrs[row][i]);
...
unsigned sub_80438A0 (unsigned char row) {
  unsigned char i, count = 0;
  for (i = 0; i < 5; i++) {
    SetCardInfo(gTurnZonePtrs[row][i]->id);
...
unsigned GetNumCardsUnlockedInRow (unsigned char row) {
  unsigned char i, count = 0;
  for (i = 0; i < 5; i++) {
    if (gTurnZonePtrs[row][i]->id != CARD_NONE && !gTurnZonePtrs[row][i]->isLocked)
      count++;
  }
  return count;
}
...
void sub_8043BC8 (struct Temp* arg0) {
  gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]->id = arg0->unk2;
  gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]->isFaceUp = 1;
  gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]->isLocked = 0;
  gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]->isDefending = 0;
  gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]->unkTwo = 0;
  gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]->unk4 = 0;
  ResetPermStage(gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]);
  ResetTempStage(gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]);
  gTurnZonePtrs[gDuelCursor.currentY][gDuelCursor.currentX]->willChangeSides = 0;
}
